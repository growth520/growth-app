-- =====================================================
-- CHECK AND FIX TABLES (DYNAMIC SCHEMA DETECTION)
-- =====================================================
-- This script first checks the actual table structure and then creates the correct schema

-- 1. CHECK EXISTING TABLE STRUCTURES
-- =====================================================

-- Check if challenge_packs table exists and its structure
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'challenge_packs') THEN
        RAISE NOTICE 'challenge_packs table exists';
    ELSE
        RAISE NOTICE 'challenge_packs table does not exist';
    END IF;
END $$;

-- Check if user_pack_progress table exists and its structure
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_pack_progress') THEN
        RAISE NOTICE 'user_pack_progress table exists';
        -- Check pack_id column type
        IF EXISTS (
            ...
            RAISE NOTICE 'user_pack_progress.pack_id is UUID type';
        ELSIF EXISTS (
            SELECT FROM information_schema.columns 
            WHERE table_schema = 'public' 
            AND table_name = 'user_pack_progress' 
            ...
            RAISE NOTICE 'user_pack_progress.pack_id is BIGINT type';
        ELSE
            RAISE NOTICE 'user_pack_progress.pack_id column not found or different type';
        END IF;
    ELSE
        RAISE NOTICE 'user_pack_progress table does not exist';
    END IF;
END $$;

-- 2. CREATE TABLES WITH CORRECT SCHEMA (BASED ON ACTUAL STRUCTURE)
-- =====================================================

-- Create challenge_packs table (if missing)
...
-- Create user_pack_progress table (if missing) - using UUID for pack_id to match existing structure
CREATE TABLE IF NOT EXISTS public.user_pack_progress (
    id BIGSERIAL PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    pack_id UUID, -- Using UUID to match the error message
...
-- Create user_pack_challenge_progress table (if missing)
...
-- 7. INSERT SAMPLE DATA FOR TESTING (WITH UUID PACK_ID)
-- =====================================================
...
-- Insert a sample user pack progress for the current user (using UUID for pack_id)
INSERT INTO public.user_pack_progress (user_id, pack_id, started_at, is_completed, completion_percentage, current_day)
VALUES (
    '6b18fb7e-5821-4a30-bad7-0d3c6873cc77'::UUID,
    gen_random_uuid(), -- Generate a random UUID for pack_id
    NOW(),
    false,
    0,
    1
