...
-- 4. Create a function to get leaderboard data with correct challenge counts
...
-- 6. Create a function to get community stats with correct challenge counts
...
BEGIN
  RETURN QUERY
  SELECT 
    (SELECT COUNT(*) FROM profiles WHERE has_completed_assessment = true) as total_active_members,
    COALESCE(SUM(up.xp), 0) as total_xp,
    COALESCE(SUM(cc.challenge_count), 0) as total_completed_challenges,
    COUNT(CASE WHEN up.streak > 0 THEN 1 END) as total_active_streaks
  FROM user_progress up
  LEFT JOIN (
    SELECT 
      user_id,
