-- Fixed RPC function for ranked feed - resolves CASE type mismatch in ORDER BY
...
BEGIN
  ...
  ORDER BY 
    CASE 
      ...
      WHEN p_filter = 'new' THEN EXTRACT(EPOCH FROM p.created_at)::numeric
      WHEN p_filter = 'most_uplifting' THEN (COALESCE(p.likes_count, 0) + COALESCE(p.comments_count, 0))::numeric
      ELSE (
        (COALESCE(p.likes_count, 0) * 2) + 
        (COALESCE(p.comments_count, 0) * 3) + 
        (COALESCE(p.shares_count, 0) * 4) + 
