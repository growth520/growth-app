-- =====================================================
-- FIX MISSING TABLES AND RELATIONSHIPS (SYNTAX FIXED)
-- =====================================================
...
-- 11. CREATE BASIC RLS POLICIES (WITHOUT IF NOT EXISTS)
-- =====================================================

-- Drop existing policies first to avoid conflicts
DROP POLICY IF EXISTS "Challenge packs are viewable by everyone" ON public.challenge_packs;
DROP POLICY IF EXISTS "Users can view own pack progress" ON public.user_pack_progress;
DROP POLICY IF EXISTS "Users can insert own pack progress" ON public.user_pack_progress;
DROP POLICY IF EXISTS "Users can update own pack progress" ON public.user_pack_progress;
...
CREATE POLICY "Challenge packs are viewable by everyone" ON public.challenge_packs
    FOR SELECT USING (true);

-- User pack progress policies (user can only see their own)
CREATE POLICY "Users can view own pack progress" ON public.user_pack_progress
    FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert own pack progress" ON public.user_pack_progress
    FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own pack progress" ON public.user_pack_progress
